...
conn = sqlite3.connect("bi_assistant.db")
cursor = conn.cursor()
...
# Generate 100+ rows of campaign data
campaign_id = 1
for product_id in range(1, 6):  # 5 products
    for month in range(1, 21):  # 20 months = 100 campaigns total
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (product_id, m, year, campaign_name, start_date.date(), end_date.date(), random.choice(["active","paused","completed"])))
...
# Quick check: print how many campaigns inserted
cursor.execute("SELECT COUNT(*) FROM campaign_months")
print("Total campaign months:", cursor.fetchone()[0])

cursor.execute("SELECT COUNT(*) FROM marketing_metrics")
print("Total marketing metrics:", cursor.fetchone()[0])

cursor.execute("SELECT COUNT(*) FROM budgets")
print("Total budgets:", cursor.fetchone()[0])
